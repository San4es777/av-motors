...
function render(){
 ...
 $('#products').innerHTML=result.length?result.slice(0,state.limit).map(p=>productMarkup(p,Core.query(state))).join(''):`<div class="empty"><h3>За цими умовами товарів немає</h3><p>Спробуйте прибрати один фільтр або перевірте артикул. Відсутність у демо-каталозі не означає, що товар неможливо замовити.</p>${state.q?'<button class="primary" data-search-all>Шукати в усьому каталозі</button>':''}<button class="secondary" data-reset="all">Скинути фільтри й пошук</button><button class="secondary" data-open-request>Підготувати запит на деталь</button></div>`;
...
document.addEventListener('click',e=>{
 ...
 if(b.matches('[data-category]')){e.preventDefault();state.cat=b.dataset.category;state.type=state.brand=state.parameter=state.volume=state.capacity=state.approval=state.standard=state.diameter='all';state.limit=12;render();document.querySelector(`[data-category="${b.dataset.category}"]`)?.focus({preventScroll:true});}
